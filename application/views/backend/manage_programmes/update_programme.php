<?php
	if($programme_data->num_rows()==1){
		$id=$programme_data->row()->id;
		$name=$programme_data->row()->name;
		$description=$programme_data->row()->description;
		$status=$programme_data->row()->status;
		
	}else{
		redirect('programmes');
	}
?>
<a href="<?php echo base_url();?>manage_programmes"​ class="btn btn-primary btn-sm">
...
<div class="row">
	<div class="col-xs-12">
		...
		?>
		<form class="form-horizontal" role="form" method="post" action="<?php echo base_url(); ?>manage_programmes/update_programme/<?php echo $id;?>">
