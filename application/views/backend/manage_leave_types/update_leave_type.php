<?php
	if($leave_type_data->num_rows()==1){
		$id=$leave_type_data->row()->id;
		$name=$leave_type_data->row()->name;
		$description=$leave_type_data->row()->description;
		$status=$leave_type_data->row()->status;
		
	}else{
		redirect('leave_types');
	}
?>
<a href="<?php echo base_url();?>manage_leave_types"​ class="btn btn-primary btn-sm">
...
<div class="row">
	<div class="col-xs-12">
		...
		?>
		<form class="form-horizontal" role="form" method="post" action="<?php echo base_url(); ?>manage_leave_types/update_leave_type/<?php echo $id;?>">
